...
- automerge対象（minor / patch）はリリース後14日の経過を確認する（無人マージのためサプライチェーン待機期間を長めに取る）
- majorは`dependencyDashboardApproval`により、Dependency Dashboardで承認するまでPRを作成しない（1件ずつレビュー）。リリース後5日の経過も確認する
